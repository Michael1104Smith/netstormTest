...
export async function setupContracts(account, library, chainId) {
  ...
  const jsonRpcProvider = new ethers.providers.StaticJsonRpcProvider(
    process.env.REACT_APP_ETHEREUM_RPC_PROVIDER,
    { chainId: parseInt(process.env.REACT_APP_ETHEREUM_RPC_CHAIN_ID) }
  )
  ...
  if (process.env.REACT_APP_ENABLE_LIQUIDITY_MINING === 'true') {
